...
`task_context::spawn`、`post` 和直接事件处理器越过任务边界的未捕获异常会统一进入一次性 shutdown 流程。停机状态一旦锁存，Robot 和 DJI 电机都会拒绝重新使能；主入口先切到 `NoForce`，为每个 DJI 分组显式排入零电流快照，并保留 20 ms 的有界发送窗口，随后才停止事件循环。该流程不能证明零帧已到达失效的总线或执行器，实车仍必须依赖设备超时、外部急停和台架验证。
