...
DJI M2006/M3508 的反馈 ID 为 `0x200 + id`，M6020 为 `0x204 + id`。M2006/M3508 允许 ID 1～8，M6020 允许 ID 1～7；对应命令绝对值上限分别为 10000、16384、30000，配置预检、对象构造和发送饱和共同执行该边界。分组命令根据型号和 ID 落到 `0x200 / 0x1ff / 0x2ff` 的四个 16 位槽；配置预检和运行时注册都会拒绝槽位冲突。反馈把 8192 计数换算为角度，把 RPM 按减速比换算为输出轴角速度，随后速度 PID 更新电流命令。`flush_commands_once()` 供安全停机路径显式刷新一次分组快照；正常运行仍由 1 ms 周期任务发送。

`set(speed)` 目前只更新 PID 目标；反馈回调到达时才计算新输出。非有限目标会被拒绝并清零 PID/电流。禁用期间的反馈只更新测量和在线心跳，不计算或缓存命令；重新使能会再次清空 PID，并在使能后的新目标和后续反馈到达前保持零输出。全局停机锁存后 `enable()` / `set_enabled(true)` 都不能绕过该门。PID 的 `dt` 来自相邻有效反馈的实际 `steady_clock` 间隔；首帧以零 `dt` 更新，间隔超过五个配置周期时保留目标并清除 PID 历史，再以零 `dt` 恢复比例控制。PID 浮点输出在转为 16 位电流命令前会检查有限性并按型号命令范围饱和。`dji_motor_group` 每 1 ms 聚合同一 CAN 的当前值，禁用或超时电机自动写零。2 ms 离线阈值与实际总线负载高度相关，调整前必须测量反馈周期与抖动。

DJI 反馈的固定 8 字节布局和解析仅定义在 `src/device/motor/dji.cpp`，不作为通用电机基类接口。当前仍使用 packed 结构解释，外部协议的端序与字段来源需要进一步写成独立 codec。成熟度：**已接入但仍需硬件验证**。
...
`serial_imu` 在串口 key 1 注册固定 packed 结构，接收 yaw/pitch/roll 及对应角速度。六个浮点字段必须全部有限；含 `NaN` 或无穷值的报文会被丢弃且不会刷新心跳。角度由度转换为 rad 并归一化，角速度还除以 1000，表明上游字段按“度/毫秒”解释；每次有效回调调用 `tick()`，离线阈值 100 ms。加速度数组当前没有数据来源。
...
`control_pad` 在串口 key 2 注册控制输入。正式抽象只包含 DJI 接收机手册中的 `CH0`～`CH3`、独立的云台俯仰拨轮以及 `S1`/`S2`；通过 `control_channel`、`channel()` 和 `gimbal_pitch_wheel()` 访问。通道值必须位于 `[-660, 660]`，开关必须是 `1`～`3`，鼠标按键必须是 `0/1`，鼠标轴和键盘掩码也必须落在各自 16 位协议范围；非法报文不会覆盖最近快照、触发更新回调或刷新心跳。`control_pad_state` 目前仍保留鼠标/键盘字段作为旧协议的兼容存储，控制逻辑不得再依赖这些字段来定义新的设备接口。有效报文刷新心跳，离线阈值为 100 ms。
...
`super_cap` 是单例设备，通过 CAN ID `0x51` 接收固定 8 字节的错误码、底盘功率、限制和能量，通过 `0x61` 发送启用位、功率限制与固定字段。对象以 100 ms 作为离线阈值；长度正确且底盘功率为有限值的反馈才更新状态和 `tick()`。状态与发送缓冲均从零初始化，但当前仍未在 `src/main.cpp` 初始化。
