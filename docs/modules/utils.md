...
`Matrix.hpp` 是固定尺寸矩阵模板，默认构造会清零内部数据，`row()`/`col()` 可被正常实例化；`RLS.hpp` 在其上实现递归最小二乘，当前主要为未接入的功率管理草稿服务。RLS 会拒绝非法遗忘因子并初始化输出，但仍缺少系统化数值测试、条件数/发散监控和与主流程的集成。修改时不要只验证“能编译”，需要构造已知解、边界和数值稳定性测试。
