...
- 致命异常和 `SIGINT` / `SIGTERM` 都只触发同一个一次性 shutdown handler。Core 不依赖控制层；停机请求会先锁存 `shutdown_requested()`，使控制层和 DJI 电机拒绝重新使能。主入口注册的 handler 再把 Robot 切到 `NoForce`，对每个 DJI 分组显式排入一次当前零电流快照，并保留 20 ms 的有界发送窗口，handler 完成后才停止事件循环。致命异常返回 1，信号退出返回 `128 + signal`。未注册 handler 的通用程序会立即停止，带执行器的入口必须在 `run()` 前注册。若总线或 writer 已经失效，软件无法保证零帧到达执行器，实车仍必须依赖电机/总线超时和外部急停。
