...
切换到 `NoForce` 会调用所有已绑定控制子系统电机的 `disable()`，清空 PID、当前输出并关闭 `enabled_`；切换到任意其他枚举值只使能底盘、云台和发射实际绑定的电机，未接入控制逻辑的配置电机不会被误启用。键鼠映射保留旧工程的 WASD、R 自旋切换、F 摩擦轮切换、鼠标云台与开火；遥控映射保留通道缩放、S1 自旋、S2 摩擦轮和滚轮开火。所有非零命令都受解锁门限制。
