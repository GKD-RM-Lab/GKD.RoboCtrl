...
bool trigger_jammed(fp32 feedback_current,
                    fp32 feedback_rpm,
                    fp32 current_threshold,
                    fp32 speed_threshold);
bool trigger_feed_allowed(bool firing,
...
void test_shoot_interlocks() {
    using roboctrl::ctrl::detail::trigger_feed_allowed;
    using roboctrl::ctrl::detail::trigger_jammed;

    CHECK(trigger_jammed(4500.0f, 0.5f, 4000.0f, 1.0f));
    CHECK(trigger_jammed(-4500.0f, -0.5f, 4000.0f, 1.0f));
    CHECK(!trigger_jammed(3500.0f, 0.5f, 4000.0f, 1.0f));
    CHECK(!trigger_jammed(4500.0f, 1.5f, 4000.0f, 1.0f));
    CHECK(trigger_feed_allowed(true, true, true, true, false));
    CHECK(!trigger_feed_allowed(true, true, true, true, true));
    CHECK(!trigger_feed_allowed(true, false, true, true, false));
    CHECK(!trigger_feed_allowed(true, true, false, true, false));
    CHECK(!trigger_feed_allowed(true, true, true, false, false));
}
...
void test_runtime_configuration_files() {
    ...
    for (const auto* path : {
        ...
        if (std::string_view{path} == "configs/sentry.yaml") {
            ...
            CHECK(config->serials.size() == 2);
            CHECK(config->dji_motors.size() == 11);
